...
function generate_id_kendaraan($koneksi){
  $prefix = "DR";
  $q = mysqli_query($koneksi, "
    SELECT MAX(CAST(SUBSTRING(id_kendaraan, 4, 3) AS UNSIGNED)) AS max_no
    FROM tb_kendaraan
    WHERE id_kendaraan LIKE 'DR___'
  ");
  $row = mysqli_fetch_assoc($q);
  $max = (int)($row['max_no'] ?? 0);
  $next = $max + 1;
  return $prefix . str_pad((string)$next, 3, "0", STR_PAD_LEFT); // DR001 dst
}

$id_kendaraan = generate_id_kendaraan($koneksi);
?>
<!DOCTYPE html>
<html lang="en">
<head>
          ...
          <div class="page-heading">
            <div class="page-heading-copy">
              ...
              <div>
                ...
                <h1 class="h3 mb-1">Data kendaraan</h1>
                <p class="text-muted mb-0">Menambahkan data kendaraan.</p>
              </div>
            </div>
            <div class="heading-actions"><a class="btn btn-outline-secondary btn-sm" href="kendaraan.php"><i class="bi bi-arrow-left" aria-hidden="true"></i> Kembali</a></div>
          </div>

          <section class="row g-3">
            <div class="col-12 col-xl-8">
              <form class="panel needs-validation" method="POST" action="proses_tambah_data_kendaraan.php" novalidate>
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">ID kendaraan</label>
                        <input class="form-control" type="text" id="id_kendaraan" name="id_kendaraan" value="<?= $id_kendaraan; ?>" readonly>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Merk kendaraan</label>
                        <input class="form-control" type="text" id="merk_kendaraan" name="merk_kendaraan" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Plat nomor</label>
                        <input class="form-control" type="text" id="platnomor" name="platnomor" required>
                    </div>
                </div>
